...
JOIN 
    movie_revenues r ON m.movie_id = r.movie_id
GROUP BY 
    d.director_id
HAVING 
    COUNT(m.movie_id) >= 2 
    AND SUM(r.domestic_revenue + r.international_revenue) > 500000000;
...
JOIN 
    movie_genres mg ON cm.movie_id = mg.movie_id
JOIN 
    genres g ON mg.genre_id = g.genre_id
JOIN 
    actor_awards aa ON a.actor_id = aa.actor_id
JOIN 
    awards aw ON aa.award_id = aw.award_id
GROUP BY 
    a.actor_id, a.first_name, a.last_name, g.name, aw.name
HAVING 
    COUNT(DISTINCT mg.genre_id) > 2 
    AND COUNT(DISTINCT aa.award_id) >= 1;
...
JOIN 
    movie_production_companies mpc ON pc.company_id = mpc.company_id
JOIN 
    movies m ON mpc.movie_id = m.movie_id
WHERE 
    EXTRACT(YEAR FROM m.release_date) > 2015
GROUP BY 
    pc.company_id, pc.name
HAVING 
    SUM(mpc.investment_amount) > 100000000;
...
FROM 
    movie_locations ml
JOIN 
    movies m ON ml.movie_id = m.movie_id
GROUP BY 
    ml.country
HAVING 
    COUNT(m.movie_id) > 2
...
FROM 
	...
	JOIN 
    movies m ON m.movie_id = mg.movie_id
	LEFT JOIN 
    movie_awards ma ON m.movie_id = ma.movie_id
	LEFT JOIN 
...
HAVING 
    ...
    AND COUNT(CASE WHEN a.award_type = 'Oscar' THEN 1 END) > 0;
...
SELECT 
    a.first_name,
    a.last_name,
    COUNT(cm.movie_id) AS lead_roles,
...
JOIN 
    cast_members cm ON a.actor_id = cm.actor_id
JOIN 
    movie_revenues mr ON cm.movie_id = mr.movie_id
WHERE 
    cm.is_lead_role = true
GROUP BY 
    a.actor_id
HAVING 
    COUNT(cm.movie_id) > 2
...
SELECT 
    ...
    ((SUM(mr.domestic_revenue + mr.international_revenue) - m.budget) / m.budget * 100) AS roi_percentage
FROM 
    movies m
JOIN 
